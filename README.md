...
- Codex Analyst Instance launches through `scripts/start_codex_analyst.sh` and performs Workflow B Phase 1/2 handoff.
- Optional n8n webhook dispatch is enabled with `N8N_WEBHOOK_URL`.
- Admin Operating Hub is the intended output layer; Notion is not required for Analyst handoff runs.
